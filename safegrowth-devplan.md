...
| 1.1 | 2026-03 | Replace liboqs with `crypto/mlkem` + `trailofbits/ml-dsa` (zero CGO); replace gorocksdb with Pebble; replace arkworks-rs/bellman with gnark; fix PoR challenge seed architecture; add co-bootstrap task to Week 8; update risk register; bump Go minimum to 1.24 |
...
│   ├── storage/             # Shard storage engine (Pebble)
...
- [ ] Implement `identity` package
  - Generate ML-DSA-65 keypair using `github.com/trailofbits/ml-dsa/mldsa65` on first run
  - Generate ML-KEM-768 keypair using `crypto/mlkem` (Go stdlib ≥ 1.24) on first run
  - Persist both keypairs to `~/.safegrowth/identity` encrypted with AES-256-GCM + Argon2id passphrase
  - Load and unlock on startup
  - Expose `NodeID()` → stable string derived from ML-DSA public key hash
  - **Note**: both libraries are pure Go, zero CGO — verify `CGO_ENABLED=0 go build` passes in CI from day one
- [ ] Implement `storage` package
  - Wrap Pebble (`github.com/cockroachdb/pebble`) with: `PutShard(id, data)`, `GetShard(id)`, `DeleteShard(id)`, `HasShard(id)`
  - Maintain shard manifest in a separate Pebble key prefix (`manifest/`)
  - Enforce configurable storage quota
  - **Why Pebble over RocksDB**: pure Go, zero CGO, RocksDB-compatible API, lower memory footprint, WAL-backed for crash safety — compiles with `CGO_ENABLED=0` on all targets
...
**Deliverable**: `safegrowth-node` binary that starts, generates identity, stores shards via REST API. All builds must pass with `CGO_ENABLED=0`.
...
- [ ] Implement `crypto` package
  - `EncryptFile(data []byte, recipientPubKey) → ciphertext` using AES-256-GCM with ML-KEM-768 key encapsulation via `crypto/mlkem`
  - `DecryptFile(ciphertext []byte, privKey) → data`
  - `SignMessage(msg []byte, privKey) → signature` using ML-DSA-65 via `github.com/trailofbits/ml-dsa/mldsa65`
  - `VerifySignature(msg, sig []byte, pubKey) → bool`
  - Hash utilities: `SHA3256(data)` via `crypto/sha3`, `BLAKE3(data)` via `github.com/zeebo/blake3`
  - Key derivation: `DeriveKey(passphrase, salt)` via `golang.org/x/crypto/argon2` (Argon2id)
  - Session key derivation: `HKDF(ikm, salt, info)` via `crypto/hkdf` (Go stdlib ≥ 1.24)
  - **All imports pure Go — confirm zero CGO with `CGO_ENABLED=0 go test ./crypto/...`**
...
- [ ] Cross-compile for Windows and Android (Termux)
  ```bash
  CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -o dist/safegrowth-node.exe ./node/cmd
  CGO_ENABLED=0 GOOS=linux   GOARCH=arm64 go build -o dist/safegrowth-node-arm64 ./node/cmd
  ```
  Verify both binaries run on target hardware. CGO_ENABLED=0 is mandatory — if any build fails with it set, a CGO dependency has been introduced and must be resolved before merge.
...
- [ ] Implement bootstrap peer config in `~/.safegrowth/config.json`
  ...
  {
    "bootstrap_peers": [
      "yourname.duckdns.org:4001",
      "friend1name.duckdns.org:4001"
    ],
    "erasure_scheme": {"data": 3, "parity": 2}
  }
  ```
  **Phase 2 requires a minimum of 2 bootstrap peers (PD-06 / NF-08)**. At the point Friend 1 joins, immediately set up their anchor node as a co-bootstrap:
  - Register a DuckDNS hostname for Friend 1's anchor node
  - Add a cron job on Friend 1's machine to update the DDNS record every 5 minutes
  - Open port 4001 UDP on Friend 1's router
  - Update `bootstrap_peers` on all nodes to include both entries
  - Daemon must log a startup warning if fewer than 2 bootstrap peers are reachable
...
- [ ] Implement credit ledger
  - Append-only log stored in Pebble (same embedded store as shard storage, separate key prefix `credits/`)
...
- [ ] Implement PoR challenge seed establishment
  - On shard placement, perform ML-KEM key exchange between shard holder and a quorum of at least 3 verifying peers to establish a shared challenge seed
  - Store only the challenge seed (not pre-computed answers) encrypted alongside shard metadata on the holder node
  - Gossip verifier candidate list (the 3+ peers holding the seed) so challenge issuance is distributed
  - **Security rationale**: verifiers derive challenge parameters (start, end, nonce) fresh from the shared seed independently; the shard holder cannot precompute answers without the actual shard because challenge parameters are not known in advance
- [ ] Implement PoR challenge issuance
  - Verifier peer derives challenge parameters `{start, end, nonce}` from the shared seed + a fresh random salt
  - Sends `{salt, expected_hash}` to the shard holder; expects response within 5 seconds
  - Shard holder computes `hash(bytes[start:end] XOR nonce)` live from the actual shard and returns it
  - Verifier confirms response matches its own independently computed expected hash
...
- [ ] Integrate `github.com/consensys/gnark` for zk-SNARK operations
  - gnark is pure Go, zero CGO — consistent with our CGO_ENABLED=0 build constraint
  - **Benchmark gate (do this first, before writing any circuit code)**:
    - Write a minimal test circuit (prove knowledge of a SHA3-256 preimage)
    - Measure Groth16 prove time on the OptiPlex 990 and Raspberry Pi 3B
    - Target: ≤ 2 seconds on OptiPlex 990; ≤ 10 seconds on Pi 3B
    - If OptiPlex target is missed: evaluate PLONK backend (faster proving, larger proof) or Bulletproofs
    - Document benchmark results in `docs/benchmarks.md` before proceeding
  - Only proceed to ZK-PoR circuit implementation after benchmark gate is passed
...
- [ ] Performance test: measure gnark proof generation time on OptiPlex 990 (target: under 2 seconds per proof); document in `docs/benchmarks.md`
...
jobs:
  build:
    ...
    steps:
      ...
      - Upload artifacts (Linux amd64, Linux arm64, Windows, Android binaries)
      # Note: CGO_ENABLED=0 is enforced on every target. Any PR that introduces
      # a CGO dependency will fail CI at this step. No C toolchain is installed
      # on the CI runner — this is intentional.
```
...
|---|---|---|---|
| trailofbits/ml-dsa API changes between releases | Low | Low | Pin to a specific tagged version in go.mod; review changelog before upgrading |
| gnark proof generation exceeds 2s target on OptiPlex 990 | Medium | Medium | Run benchmark gate at start of Week 15 before writing circuit code; fallback options: PLONK backend (faster proving), proof batching, or defer ZK-PoR to post-MVP |
