...
**White Paper v2.1 — 2025–2026 AI Era**

> **Revision note (v2.1)**: Updated technology stack to reflect zero-CGO library decisions. ML-KEM now uses `crypto/mlkem` (Go standard library ≥ 1.24). ML-DSA now uses `github.com/trailofbits/ml-dsa` (pure Go, Trail of Bits). ZK proofs now use `github.com/consensys/gnark` (pure Go). Local storage now uses `github.com/cockroachdb/pebble` (pure Go). These changes eliminate all CGO dependencies from the node daemon, enabling clean cross-compilation to all target platforms with a single build command and no C toolchain.
...
- **Storage engine**: manages encrypted shard storage on local disk using Pebble (`github.com/cockroachdb/pebble`), a pure Go embedded key-value store with WAL-backed crash safety and a RocksDB-compatible API
- **Encryption layer**: AES-256-GCM for data at rest; ML-KEM (CRYSTALS-Kyber) for key exchange via `crypto/mlkem` (Go standard library ≥ 1.24, pure Go, zero CGO); ML-DSA (CRYSTALS-Dilithium) for message signing via `github.com/trailofbits/ml-dsa` (pure Go, Trail of Bits, zero CGO) — all per NIST FIPS 203/204
- **Sharding engine**: splits files into erasure-coded chunks using Reed-Solomon coding (`github.com/klauspost/reedsolomon` — pure Go, SIMD-accelerated, zero CGO); default scheme is 5-of-9 for open networks, 2-of-3 for local testing
- **ZK proof module**: generates and verifies zero-knowledge proofs of storage using zk-SNARKs (`github.com/consensys/gnark` — pure Go, zero CGO, Groth16 and PLONK backends), enabling storage verification without content exposure
